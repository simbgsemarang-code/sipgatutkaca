...
<div class="info-card"><h3>Riwayat Pengajuan ITR</h3><?php if(!$pengajuan_itr): ?><p>Belum ada pengajuan. Mulai melalui menu PENGAJUAN ITR.</p><?php else: ?><div style="overflow:auto"><table><thead><tr><th>Hasil ITR</th><th>No. Permohonan</th><th>Jenis</th><th>Lokasi</th><th>Tanggal</th><th>Status</th><th>Aksi</th></tr></thead><tbody><?php foreach($pengajuan_itr as $r): $perusahaan=($r['jenis_pemohon']??'perorangan')==='perusahaan'; $berkas=array('file_permohonan'=>'Permohonan','file_ktp'=>'KTP','file_sertifikat'=>'Sertifikat','file_siteplan'=>'Site plan','file_denah_foto'=>'Denah & Foto'); if($perusahaan)$berkas+=array('file_nib'=>'NIB','file_npwp'=>'NPWP','file_akta'=>'Akta'); $kurang=0; foreach($berkas as $field=>$label){ if(empty($r[$field])) $kurang++; } ?><tr><td><?php if(!empty($r['file_hasil_itr'])): ?><a href="<?= base_url('pemohon/hasil_itr/'.$r['id']) ?>" style="font-weight:700" target="_blank">Unduh Hasil ITR</a><?php else: ?><span style="color:var(--muted)">Belum tersedia</span><?php endif; ?></td><td><?= htmlspecialchars($r['no_permohonan']) ?></td><td><?= $perusahaan?'Perusahaan':'Perorangan' ?></td><td><?= htmlspecialchars($r['alamat_lokasi']) ?></td><td><?= date('d/m/Y H:i',strtotime($r['created_at'])) ?></td><td><?= htmlspecialchars(ucfirst($r['status'])) ?></td><td><select class="itr-aksi-select" onchange="itrAksi(this)">
  <option value="">Pilih Aksi</option>
  <option value="<?= base_url('pemohon/upload-berkas-itr/'.$r['id']) ?>"><?= $kurang?'Lengkapi Berkas ('.$kurang.' kurang)':'Kelola Berkas' ?></option>
  ...
  <?php endif; ?>
</select></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></div>
<script>
