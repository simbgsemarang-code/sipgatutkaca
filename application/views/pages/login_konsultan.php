...
<title>Login Tim Konsultan — SIP Gatutkaca · Kabupaten Cilacap</title>
...
<section class="hero page">
  <div class="hero-bg" style="background-image:url('https://commons.wikimedia.org/wiki/Special:FilePath/Barracks,_Benteng_Pendem,_Cilacap_2015-03-21.jpg?width=1200')" role="img" aria-label="Pemandangan Kabupaten Cilacap"></div>
  <div class="wrap">
...
<section>
  <div class="wrap">
    <div class="form-card center reveal">
      <p class="eyebrow" style="margin-bottom:8px">Portal Tim Konsultan</p>
      ...
      <div class="field"><label for="tk-pass">Kata Sandi</label><input id="tk-pass" type="password" placeholder="••••••••"></div>
      <button class="btn btn-gold" type="button" style="width:100%" id="tkMasuk">Masuk sebagai Tim</button>
      <p class="note" style="text-align:center">Akses terbatas bagi anggota tim yang ditugaskan. Bukan petugas? <a href="<?php echo base_url('login'); ?>" style="color:var(--gold-300);text-decoration:underline">Masuk sebagai pemohon</a>.</p>
      <p class="note" style="text-align:center">Contoh antarmuka — autentikasi aktif setelah terhubung ke sistem kepegawaian.</p>
    </div>
  </div>
</section>

<style>
.tim-switch{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:28px}
.tim-opt{border:1px solid var(--line);background:var(--input);color:var(--muted);padding:16px 10px;cursor:pointer;display:grid;gap:4px;justify-items:center;font-family:var(--body);transition:.25s}
.tim-opt b{font-family:var(--display);font-weight:400;font-size:1.3rem;letter-spacing:.24em;color:var(--text)}
.tim-opt span{font-size:.62rem;letter-spacing:.16em;text-transform:uppercase}
.tim-opt:hover{border-color:var(--gold-500)}
.tim-opt.sel{border-color:var(--gold-500);box-shadow:0 0 0 1px var(--gold-500)}
.tim-opt.sel b{color:var(--gold-300)}
</style>
...
(function(){
  ...
  function pilih(t){
    cur=t;
    document.querySelectorAll(".tim-opt").forEach(function(b){b.classList.toggle("sel",b.dataset.tim===t)});
    desc.textContent="Anda akan masuk sebagai "+NAMA[t]+" ("+t+").";
    btn.textContent="Masuk sebagai "+t;
  }
  document.querySelectorAll(".tim-opt").forEach(function(b){
    b.addEventListener("click",function(){pilih(b.dataset.tim)});
  });
  var t=new URLSearchParams(location.search).get("tim");
  if(t&&NAMA[t])pilih(t);
})();
</script>
