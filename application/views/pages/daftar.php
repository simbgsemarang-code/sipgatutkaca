...
<title>Daftar Akun — SIP Gatutkaca · Kabupaten Cilacap</title>
...
/* ===== FORM ===== */
.form-card{background:var(--surface);border:1px solid var(--line);padding:46px;max-width:560px}
...
.hint{font-size:.72rem;color:var(--muted);margin-top:7px}
...
<section style="padding-top:calc(84px + 100px)">
  <div class="wrap">
    <div class="page-breadcrumb reveal" style="max-width:560px;margin-left:auto;margin-right:auto">
      <a href="<?php echo base_url(); ?>">Beranda</a><span class="sep">/</span>Daftar
    </div>
    <form class="form-card center reveal" action="<?php echo base_url('daftar/proses'); ?>" method="post">
      <p class="eyebrow" style="margin-bottom:8px">Pendaftaran Akun Pemohon</p>
      <h2 style="font-size:1.6rem;margin-bottom:24px">Daftar Akun Baru</h2>
      <?php if (!empty($error)): ?>
        <div class="alert alert-err"><?php echo htmlspecialchars($error, ENT_QUOTES, 'UTF-8'); ?></div>
      <?php endif; ?>
      <div class="field">
        <label for="d-nama">Nama / Nama Perusahaan</label>
        <input id="d-nama" name="nama" type="text" required placeholder="Nama lengkap atau nama perusahaan" value="<?php echo isset($old['nama']) ? htmlspecialchars($old['nama'], ENT_QUOTES, 'UTF-8') : ''; ?>">
      </div>
      <div class="field">
        <label for="d-email">Email</label>
        <input id="d-email" name="email" type="email" required placeholder="email@contoh.com" value="<?php echo isset($old['email']) ? htmlspecialchars($old['email'], ENT_QUOTES, 'UTF-8') : ''; ?>">
      </div>
      <div class="field">
        <label for="d-nik">NIK / NPWP</label>
        <input id="d-nik" name="nik" type="text" inputmode="numeric" required placeholder="16 digit NIK atau 15-16 digit NPWP" value="<?php echo isset($old['nik']) ? htmlspecialchars($old['nik'], ENT_QUOTES, 'UTF-8') : ''; ?>">
        <p class="hint">Untuk perorangan isi NIK KTP, untuk perusahaan isi NPWP.</p>
      </div>
      <div class="field-row">
        <div class="field">
          <label for="d-pass">Kata Sandi</label>
          <input id="d-pass" name="password" type="password" required minlength="8" placeholder="Minimal 8 karakter">
        </div>
        <div class="field">
          <label for="d-pass2">Ulangi Kata Sandi</label>
          <input id="d-pass2" name="ulang_password" type="password" required minlength="8" placeholder="Ketik ulang kata sandi">
        </div>
      </div>
      <button class="btn btn-gold" type="submit" style="width:100%">Daftar Akun</button>
      <p class="note" style="text-align:center">Sudah memiliki akun? <a href="<?php echo base_url('login'); ?>" style="color:var(--gold-300);text-decoration:underline">Masuk di sini</a></p>
    </form>
  </div>
</section>
