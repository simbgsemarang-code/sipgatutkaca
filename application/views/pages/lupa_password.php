...
<title>Lupa Kata Sandi — SIP Gatutkaca · Kabupaten Cilacap</title>
...
<section style="padding-top:calc(84px + 100px)">
  <div class="wrap">
    <div class="page-breadcrumb reveal" style="max-width:520px;margin-left:auto;margin-right:auto">
      <a href="<?php echo base_url(); ?>">Beranda</a><span class="sep">/</span><a href="<?php echo base_url('login'); ?>">Masuk</a><span class="sep">/</span>Lupa Kata Sandi
    </div>
    <form class="form-card center reveal" action="<?php echo base_url('login/proses-lupa'); ?>" method="post">
      <p class="eyebrow" style="margin-bottom:8px">Atur Ulang Kata Sandi</p>
      <h2 style="font-size:1.6rem;margin-bottom:14px">Lupa Kata Sandi?</h2>
      <p class="note" style="margin-top:0;margin-bottom:24px">Masukkan email akun Anda. Kalau terdaftar, kami kirim tautan untuk membuat kata sandi baru.</p>
      <?php if (!empty($sukses)): ?>
      ...
      <?php endif; ?>
      <div class="field"><label for="lp-email">Email</label><input id="lp-email" name="email" type="email" required placeholder="email@contoh.com" value="<?php echo isset($old['email']) ? htmlspecialchars($old['email'], ENT_QUOTES, 'UTF-8') : ''; ?>"></div>
      <button class="btn btn-gold" type="submit" style="width:100%">Kirim Tautan Atur Ulang</button>
      <p class="note" style="text-align:center">Sudah ingat kata sandi? <a href="<?php echo base_url('login'); ?>" style="color:var(--gold-300);text-decoration:underline">Kembali ke halaman masuk</a></p>
    </form>
  </div>
</section>
