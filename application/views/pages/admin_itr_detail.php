<?php $heading='Detail Pengajuan ITR';$top_actions='';$this->load->view('pbg_pu/header',compact('heading','top_actions','nama_pengguna')); ?>
<p><a href="<?= base_url('admin_itr') ?>">&larr; Kembali ke daftar pengajuan</a></p>
...
<h3>Riwayat Informasi</h3><?php if(!$pesan): ?><p>Belum ada informasi terkirim.</p><?php else: foreach($pesan as $p): ?><div class="notice"><strong><?= htmlspecialchars($p['nama_admin']?:'Administrator') ?></strong> · <?= date('d/m/Y H:i',strtotime($p['created_at'])) ?><p><?= nl2br(htmlspecialchars($p['isi'])) ?></p><small><?= $p['dibaca_pada']?'Sudah dibaca pemohon · '.date('d/m/Y H:i',strtotime($p['dibaca_pada'])):'Belum dibaca pemohon' ?></small></div><?php endforeach; endif; ?>
</article>
