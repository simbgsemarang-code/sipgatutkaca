...
/**
 * Pengajuan SLF - loket PU menginput permohonan Sertifikat Laik
 * Fungsi atas nama warga yang datang langsung (bukan pemohon login
 * sendiri). TAHAP 1: pengisian data pemohon + data bangunan/tanah +
 * unggah dokumen teknis (as-built drawing + laporan kelaikan fungsi),
 * sampai status permohonan terkirim untuk diverifikasi TPA. Strukturnya
 * disalin persis dari Pengajuan_pbg (pola guard peran + wizard yang
 * sama). Alur lanjutan SLF (jadwal konstruksi, dokumen akhir,
 * penerbitan SK SLF + kode unik - lihat Panduan Permohonan SLF SIMBG)
 * belum tercakup - lihat catatan di database/pengajuan_slf.sql.
 */
class Pengajuan_slf extends CI_Controller {
	 ...
	 * dipakai TPA (lewat Tpa_pengajuan_slf) untuk menyaring dokumen
	...
	private $peta_dokumen = array(
		'Data Umum' => array(
			...
			'dokumen'    => array(
				...
				'kepemilikan_tanah' => 'Bukti Kepemilikan Tanah & SPPT / Keterangan NOP',
				'penyedia_jasa'     => 'Data Penyedia Jasa Pengawas / Manajemen Konstruksi',
			),
		),
		'Bidang Arsitektur & Tata Kota' => array(
			'tpa_bidang' => array('tpa_arsitek'),
			'dokumen'    => array(
				'kkpr'               => 'Dokumen KKPR / KRK / Informasi Tata Ruang (ITR)',
				'asbuilt_arsitektur' => 'Gambar As-Built Drawing Arsitektur',
				'spek_arsitektur'    => 'Spesifikasi Teknis Arsitektur Bangunan',
		...
		'Bidang Struktur & Sipil' => array(
			...
			'dokumen'    => array(
				'asbuilt_struktur' => 'Gambar As-Built Drawing Struktur',
				'spek_struktur'    => 'Spesifikasi Teknis Struktur Bangunan',
				'hitung_struktur'  => 'Perhitungan Teknis Struktur',
			),
		),
		'Bidang Mekanikal, Elektrikal & Perpipaan (MEP)' => array(
			'tpa_bidang' => array('tpa_mep'),
			'dokumen'    => array(
				'asbuilt_mep' => 'Gambar As-Built Drawing Mekanikal, Elektrikal & Plambing',
				'spek_mep'    => 'Spesifikasi Teknis Mekanikal, Elektrikal & Plambing',
			),
		),
		'Pemeriksaan Kelaikan Fungsi' => array(
			'tpa_bidang' => null,
			'dokumen'    => array(
				'laporan_kelaikan'    => 'Laporan Pemeriksaan Kelaikan Fungsi Bangunan',
				'pernyataan_kelaikan' => 'Surat Pernyataan Kelaikan Fungsi',
			),
		),
	...
	/** Sama persis dengan Tpa_pengajuan_slf::$peran_bidang - disalin, bukan dibagi lewat library. Dipakai _hitung_status_keseluruhan() di bawah. */
	private $peran_bidang = array('tpa_arsitek', 'tpa_struktur', 'tpa_mep');

	/** Sama persis dengan Tpa_pengajuan_slf::$kolom_reviewer - disalin, bukan dibagi lewat library. Peran bidang -> kolom penugasan di pengajuan_slf (lihat database/pengajuan_slf_reviewer.sql). */
	...
	/** Sama persis dengan Tpa_pengajuan_slf::$label_bidang di view - disalin, bukan dibagi lewat library. */
	...
	public function index()
	{
		...
		// Tpa_pengajuan_slf::_hitung_status_keseluruhan() untuk
		// catatan lengkap soal tabel pengajuan_slf_persetujuan_tpa.
		...
		$data['daftar']             = $this->db->where('dibuat_oleh', (int) $this->session->userdata('user_id'))->order_by('created_at', 'DESC')->get('pengajuan_slf')->result_array();
		...
		$this->load->view('pages/pengajuan_slf_list', $data);
	}
	...
	public function tambah($id = null)
	{
		...
		$data['dokumen_ada']      = $id > 0 ? $this->db->where('id_pengajuan', $id)->get('pengajuan_slf_dokumen')->result_array() : array();
		$data['error']            = $this->session->flashdata('error');
		$data['old']              = $this->session->flashdata('old');
		$data['nama_pengguna']    = $this->session->userdata('nama');

		$this->load->view('pages/pengajuan_slf_form', $data);
	}
	...
	public function lihat($id = null)
	{
		$id  = (int) $id;
		$row = $id > 0
			? $this->db->select('pengajuan_slf.*, peninjau.nama AS nama_peninjau, ra.nama AS nama_reviewer_arsitek, rs.nama AS nama_reviewer_struktur, rm.nama AS nama_reviewer_mep')
				->from('pengajuan_slf')
		...
		$dokumen = $this->db->where('id_pengajuan', $id)->get('pengajuan_slf_dokumen')->result_array();
	...
	{
		$id  = (int) $id;
		$row = $id > 0 ? $this->db->where('id', $id)->get('pengajuan_slf')->row_array() : NULL;
		...
		$this->load->view('pages/pengajuan_slf_checklist', $data);
	}
	 ...
	 * (perilaku lama) - lihat database/pengajuan_slf_reviewer.sql dan
	 * Tpa_pengajuan_slf::_bidang_boleh_akses().
	 */
	public function reviewer($id = null)
	{
		...
		$this->load->view('pages/pengajuan_slf_reviewer', $data);
	}
	...
	public function perbaiki($id = null)
	{
		...
		$this->load->view('pages/pengajuan_slf_perbaikan', $data);
	}
	...
	public function kirim_perbaikan($id = null)
	{
		...
		$this->db->where('id', $id)->update('pengajuan_slf', array(
		...
		// Tpa_pengajuan_slf::kirim_catatan() - disalin, bukan dibagi
		// lewat library).
		if ($row['status'] === 'perbaikan_dokumen_konsultasi')
		{
			$this->db->where('id_pengajuan', $id)->where_in('status', array('perbaikan_dokumen', 'perbaikan_dokumen_konsultasi'))->delete('pengajuan_slf_persetujuan_tpa');
			$status_lanjut = 'menunggu_jadwal_konsultasi';
		}
		elseif ($row['status'] === 'perbaikan_dokumen')
		{
			$this->db->where('id_pengajuan', $id)->where('status', 'perbaikan_dokumen')->delete('pengajuan_slf_persetujuan_tpa');
			$status_lanjut = $this->_hitung_status_keseluruhan($id);
		}
		else
		{
			$this->db->where('id_pengajuan', $id)->delete('pengajuan_slf_persetujuan_tpa');
			$status_lanjut = 'verifikasi_dokumen';
		}
		$this->db->where('id', $id)->update('pengajuan_slf', array('status' => $status_lanjut));
		...
		redirect('pengajuan-slf/lihat/' . $id);
	}

	/** Sama persis dengan Tpa_pengajuan_slf::_hitung_status_keseluruhan() - disalin, bukan dibagi lewat library. Lihat catatan lengkap di sana. */
	private function _hitung_status_keseluruhan($id)
	{
		$status_per_bidang = array();
		foreach ($this->db->select('bidang, status')->where('id_pengajuan', $id)->get('pengajuan_slf_persetujuan_tpa')->result_array() as $b)
		{
	 ...
	 * persis dengan Tpa_pengajuan_slf::_hitung_checklist() - disalin,
	...
	public function simpan($id = null)
	{
		...
		if ($data['nama_pemohon'] === '')
		{
			...
			redirect('pengajuan-slf/tambah' . ($id > 0 ? '/' . $id : ''));
			return;
		}
		...
		// berkas diunggah ke folder application/uploads/pengajuan_slf/{id}/.
		if ($id > 0)
		{
			$this->db->where('id', $id)->update('pengajuan_slf', $data);
		}
		else
		{
			$data['dibuat_oleh'] = (int) $this->session->userdata('user_id');
			$this->db->insert('pengajuan_slf', $data);
		...
		if ($aksi === 'kirim')
		{
			$no_reg = 'SLF-' . date('Ymd') . '-' . str_pad((string) $id, 4, '0', STR_PAD_LEFT);
			$this->db->where('id', $id)->update('pengajuan_slf', array(
		...
		redirect('pengajuan-slf/tambah/' . $id);
	}
	...
	public function hapus($id = null)
	{
		...
		$dir = APPPATH . 'uploads/pengajuan_slf/' . $id;
		if (is_dir($dir))
		{
		...
		// Baris pengajuan_slf_dokumen ikut terhapus otomatis (ON DELETE CASCADE).
		$this->db->where('id', $id)->delete('pengajuan_slf');
	...
	public function berkas($tipe = null, $id = null)
	{
		...
		if ($tipe === 'dokumen')
		{
			...
			$induk = $this->db->where('id', $dok['id_pengajuan'])->get('pengajuan_slf')->row_array();
	...
	/**
	 * $row (hasil query pengajuan_slf, kolom dibuat_oleh wajib ikut
	...
	private function _proses_unggah_dokumen($id)
	{
		...
		foreach ($_FILES['dokumen']['name'] as $slug => $nama_asli)
		{
			...
			$lama = $this->db->where('id_pengajuan', $id)->where('jenis_dokumen', $daftar_jenis[$slug])->get('pengajuan_slf_dokumen')->row_array();
			if ($lama !== NULL)
			{
				@unlink(APPPATH . 'uploads/pengajuan_slf/' . $lama['path_file']);
				$this->db->where('id', $lama['id'])->delete('pengajuan_slf_dokumen');
			}

			$this->db->insert('pengajuan_slf_dokumen', array(
				'id_pengajuan'   => $id,
