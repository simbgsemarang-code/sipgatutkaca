...
/**
 * Peninjauan Pengajuan SLF oleh TPA (Tim Profesi Ahli) - menandai
 * dokumen yang tidak sesuai dan memutuskan jenis perbaikan yang
 * perlu dilalui pemohon/PU. Melengkapi alur loket di
 * Pengajuan_slf.php (sisi PU/pemohon) dengan sisi peninjau. Disalin
 * persis dari Tpa_pengajuan_pbg (pola persetujuan per bidang yang
 * sama) - cuma tabel pengajuan_slf* dan daftar dokumen teknis (as-built
 * drawing + laporan kelaikan fungsi) yang berbeda.
 *
 * PERSETUJUAN PER BIDANG: ketiga spesialisasi TPA (Arsitektur & Tata
 * Kota, Struktur & Sipil, MEP) meninjau & memutuskan SECARA
 * INDEPENDEN lewat tabel pengajuan_slf_persetujuan_tpa - satu baris
 * per bidang per permohonan (tanpa baris = bidang itu belum
 * meninjau). Kolom pengajuan_slf.status DITURUNKAN dari isi tabel
 ...
 * Pengajuan_slf::kirim_perbaikan()). "Perbaikan Dokumen Konsultasi"
 ...
 * (jadwal konstruksi, dokumen akhir, penerbitan SK SLF + kode unik)
 * belum tercakup di sini - lihat catatan di database/pengajuan_slf.sql.
 */
class Tpa_pengajuan_slf extends CI_Controller {
	...
	/** Sama persis dengan Pengajuan_slf::$kolom_reviewer - disalin, bukan dibagi lewat library. Peran bidang -> kolom penugasan di pengajuan_slf (lihat database/pengajuan_slf_reviewer.sql). Dipakai _bidang_boleh_akses() utk membatasi akses per permohonan. */
	 ...
	 * konsultasi (lihat Pengajuan_slf::kirim_perbaikan()), menutup
	 * peninjauan per bidang lebih lanjut.
	 */
	private $status_tpa_aktif = array('verifikasi_dokumen', 'perbaikan_dokumen');

	/** Sama persis dengan Pengajuan_slf - disalin, bukan dibagi lewat library, mengikuti pola guard peran di codebase ini. */
	...
	/** Sama persis dengan Pengajuan_slf::$peta_dokumen - disalin, bukan dibagi lewat library. Dipakai di sini untuk menyaring dokumen sesuai bidang TPA yang login (lihat _grup_dokumen_untuk_peran()). */
	 ...
	 * database/pengajuan_slf_reviewer.sql.
	 */
	public function index()
	{
		...
		$data['daftar']        = $query->order_by('created_at', 'DESC')->get('pengajuan_slf')->result_array();
		$data['sukses']        = $this->session->flashdata('sukses');
		$data['error']         = $this->session->flashdata('error');
		$data['nama_pengguna'] = $this->session->userdata('nama');
		$this->load->view('pages/tpa_pengajuan_slf_list', $data);
	}
	...
	public function lihat($id = null)
	{
		...
		$row = $id > 0
			? $this->db->select('pengajuan_slf.*, peninjau.nama AS nama_peninjau, ra.nama AS nama_reviewer_arsitek, rs.nama AS nama_reviewer_struktur, rm.nama AS nama_reviewer_mep')
				...
				->where('pengajuan_slf.id', $id)->where('pengajuan_slf.status !=', 'draf')
				->get()->row_array()
			: NULL;
		...
		$dokumen_terunggah = $this->db->where('id_pengajuan', $id)->get('pengajuan_slf_dokumen')->result_array();
		...
		$data['nama_pengguna']    = $this->session->userdata('nama');
		$this->load->view('pages/tpa_pengajuan_slf_detail', $data);
	}
	...
	{
		$id  = (int) $id;
		$row = $id > 0 ? $this->db->where('id', $id)->where('status !=', 'draf')->get('pengajuan_slf')->row_array() : NULL;
		...
		$this->load->view('pages/tpa_pengajuan_slf_checklist', $data);
	}
	 ...
	 * Lihat database/pengajuan_slf_reviewer.sql dan
	 * Pengajuan_slf::reviewer()/simpan_reviewer() (tempat PU
	...
	/**
	 * Status pengajuan_slf.status DITURUNKAN dari isi
	 * pengajuan_slf_persetujuan_tpa, bukan ditulis langsung oleh satu
	 * keputusan - dipanggil ulang tiap ada keputusan baru dari TPA
	 * (kirim_catatan()) atau perbaikan dikirim PU
	 * (Pengajuan_slf::kirim_perbaikan(), yang menyalin fungsi ini).
	 ...
	 * Pengajuan_slf::_hitung_checklist() - disalin, bukan dibagi lewat
	 * library.
	 */
	...
	public function tandai_dokumen($id_dokumen = null)
	{
		...
		$dok = $this->db->where('id', $id_dokumen)->get('pengajuan_slf_dokumen')->row_array();
		...
		$permohonan = $this->db->where('id', $dok['id_pengajuan'])->get('pengajuan_slf')->row_array();
		$peran      = (string) $this->session->userdata('role');
		...
		$tujuan_ulang = 'tpa-pengajuan-slf/lihat/' . (int) $dok['id_pengajuan'];
		...
		if ($aksi === 'tolak')
		{
			...
			$this->db->where('id', $id_dokumen)->update('pengajuan_slf_dokumen', array(
				'status'            => 'ditolak',
	 ...
	 * disimpan sebagai 1 baris di pengajuan_slf_persetujuan_tpa, lalu
	 * pengajuan_slf.status dihitung ulang dari SEMUA baris lewat
	...
	{
		$id  = (int) $id;
		$row = $this->db->where('id', $id)->where_in('status', $this->status_tpa_aktif)->get('pengajuan_slf')->row_array();

		if ($row === NULL)
		{
			$this->session->set_flashdata('error', 'Permohonan tidak ditemukan atau statusnya sudah berubah - muat ulang halaman.');
			redirect('tpa-pengajuan-slf');
			return;
		}
		...
		if (! in_array($peran, $this->peran_bidang, TRUE))
		{
			...
			redirect('tpa-pengajuan-slf/lihat/' . $id);
			return;
		}
		if (! $this->_bidang_boleh_akses($row, $peran, (int) $this->session->userdata('user_id')))
		{
		...
		$sudah_memutuskan = $this->db->where('id_pengajuan', $id)->where('bidang', $peran)->get('pengajuan_slf_persetujuan_tpa')->row_array();
		...
		$this->db->insert('pengajuan_slf_persetujuan_tpa', array(
			'id_pengajuan'  => $id,
			'bidang'        => $peran,
			'status'        => $status_baru,
		...
		$this->db->where('id', $id)->update('pengajuan_slf', array('status' => $status_keseluruhan));
	 ...
	 * disalin dari Pengajuan_slf::berkas() (bukan dibagi lewat
